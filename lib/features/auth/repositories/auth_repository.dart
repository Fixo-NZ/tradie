...
class AuthRepository {
  ...
  Future<ApiResult<AuthResponse>> register(RegisterRequest request) async {
    try {
      ...
      final authResponse = AuthResponse.fromJson(response.data);
      // We set the token, but the ViewModel will decide whether to navigate to Home or Verify Email
      await _dioClient.setToken(authResponse.accessToken);
  ...
  // 2. Verify OTP
  // (RENAMED back to verifyPasswordResetOtp to fix reset_password_viewmodel error)
  Future<ApiResult<void>> verifyPasswordResetOtp(String email, String otp) async {
    try {
      await _dioClient.dio.post(
  ...
  // 3. Set New Password
  // (RENAMED back to setNewPassword to fix reset_password_viewmodel error)
